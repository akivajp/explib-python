...
def saveWordPairCounts(savePath, pairCounter):
  ...
  for pair in sorted(pairCounter.pairCounts.keys()):
    srcWord = pair[0]
    trgWord = pair[1]
    srcCount = pairCounter.srcCounts[srcWord]
    trgCount = pairCounter.trgCounts[trgWord]
    pairCount = pairCounter.pairCounts[pair]
...
def pivotWordPairCounts(cntSrcPvt, cntPvtTrg, nbest=50):
  ...
  #    print("src:%s pvt:%s" % (srcWord, pvtWord))
  #    print("trg list: %s" % cntPvtTrg
  ...
  #        cntSrcTrg.addSrc(srcWord, minCount)
  #        cntSrcTrg.addTrg(trgWord, minCount)
    if srcWord != "NULL":
...
#      cntSrcTrg.addSrc(srcWord, count)
#      cntSrcTrg.addSrc(trgWord, count)
  # 逆向きに走査してフィルタリング
#  for trgWord, srcWord in sorted(cntSrcTrg.pairCounts.items(), key = lambda pair: reversed(pair[1]))
...
#        stdout.write("SEARCHING WITH TRG: %s MAX: %d\n" % (trgWord, maxCount))
        for srcWord in cntSrcTrg.srcAligned[trgWord]:
